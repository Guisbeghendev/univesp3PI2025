...
try {
    ...
    if ($stmt->rowCount() > 0) {
        // Atualiza o status do contrato para "confirmado"
        $updateSql = "UPDATE Contratos SET status = 'confirmado' WHERE id = :id";
        $updateStmt = $conn->prepare($updateSql);
        $updateStmt->bindParam(':id', $id_contrato, PDO::PARAM_INT);
        $updateStmt->execute();

        // Mensagem de sucesso
        $msg = "Contrato aceito com sucesso!";
    } else {
        $msg = "Contrato não encontrado ou não está pendente.";
    }
} catch (PDOException $e) {
    // Se ocorrer um erro
    $msg = "Erro ao aceitar o contrato: " . $e->getMessage();
}
?>
...
<body>
    ...
    <main class="main-content">
        <section class="signup-section">
            <h3><?php echo $msg; ?></h3>
            <button onclick="window.location.href='dashboard.php'">Voltar ao Meu painel</button>
        </section>
    </main>
